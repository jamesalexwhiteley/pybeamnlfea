...
for i in range(14):
    for j in range(14):
        ...
        if i in theta_indices and j in theta_indices:
            dNi = all_derivs[i][1]
            dNj = all_derivs[j][1]
            integrand = Mx0*beta_y*dNi*dNj
            K[i,j] += sp.integrate(integrand, (z, 0, L))
            
        if (i in u_indices and j in theta_indices) or (i in theta_indices and j in u_indices):
            dNi = all_derivs[i][1]
            dNj = all_derivs[j][1]
            integrand = -Mx0*dNi*dNj
            K[i,j] += sp.integrate(integrand, (z, 0, L))
...
for i in range(14):
    for j in range(14):
        ...
        if i in theta_indices and j in theta_indices:
            dNi = all_derivs[i][1]
            dNj = all_derivs[j][1]
            integrand = -My0*beta_x*dNi*dNj
            K[i,j] += sp.integrate(integrand, (z, 0, L))
            
        if (i in v_indices and j in theta_indices) or (i in theta_indices and j in v_indices):
            dNi = all_derivs[i][1]
            dNj = all_derivs[j][1]
            integrand = -My0*dNi*dNj
            K[i,j] += sp.integrate(integrand, (z, 0, L))
