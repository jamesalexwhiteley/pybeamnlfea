...
def run_simply_supported_beam(E, G, rho, L, A, Ix, Iy, J, Iw, y0, beta_x, load_height_above_shear_center, n_elements=10):
    ...
    beam.add_section("monosym_I", Section(A=A, Iy=Ix, Iz=Iy, J=J, Iw=Iw, y0=y0, z0=0, beta_x=beta_x))
    ...
    # delta = beta_x / L * np.sqrt(E * Iy / (G * J))
    # gamma_2 = float(interp_table1((K_target, epsilon, delta)))
    # P_c = gamma_2 * scale_N
    
    return P_c

def run_cantilever(E, G, rho, L, A, Ix, Iy, J, Iw, y0, beta_x, load_height_above_shear_center, n_elements=10):
    n = n_elements
    beam = Frame()
    beam.add_nodes([[L*i/n, 0, 0] for i in range(n+1)])
...
for eps in test_epsilons:
    ...
    Pc_pos = run_cantilever(E, G, rho, L, A, Ix, Iy, J, Iw, y0_positive_delta, beta_x_positive_delta, a)
    results_cant_pos['eps'].append(eps)
    results_cant_pos['Pc_N'].append(Pc_pos)
    results_cant_pos['Pc_lb'].append(Pc_pos / lb_to_N)
    
    Pc_neg = run_cantilever(E, G, rho, L, A, Ix, Iy, J, Iw, y0_negative_delta, beta_x_negative_delta, a)
