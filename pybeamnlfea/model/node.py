...
class Node:
    """
    Class representing a node in a structural FEA model.
    
    A node is a point in 3D space with 6 degrees of freedom (DOFs):
    - 3 translational DOFs (displacements in x, y, z directions)
    - 3 rotational DOFs (rotations about x, y, z axes)
    """
    def __init__(self, id: int, coordinates: np.ndarray):
        ...
        if len(self.coordinates) != 3:
            raise ValueError("Node coordinates must be a 3D vector [x, y, z]")
        
        # Initial displacements are zero
        # [dx, dy, dz, rx, ry, rz] where d=translation, r=rotation
        self.displacements = np.zeros(6)
        
        # For storing applied loads at this node
        self.applied_loads = np.zeros(6)
