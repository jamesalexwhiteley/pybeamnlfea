...
class Frame:
    ...
    # def show_mode_shape(self, mode, scale: float=1.0, show_undeformed: bool=True, show_local_axes: bool=False) -> None: 
    #     """
    #     Plot the deformed mode shape with proper normalization and scaling.
    #     """
    #     # Reset state first
    #     self.reset_state()
        
    #     # Convert mode to dictionary if it's not already
    #     displacements = {}
    ...
    #                     displacements[(node_id, dof)] = mode[idx]
    #     else:
    #         displacements = mode
        
    #     # Find max translational displacement for normalization
    #     trans_disps = []
    #     for node_id in self.nodes:
    #         node_disp = []
    #         for dof in range(3):  # First 3 DOFs are translational
    #             val = displacements.get((node_id, dof), 0.0)
    #             node_disp.append(abs(val) if val is not None else 0.0)
    #         trans_disps.append(np.linalg.norm(node_disp))
        
    #     max_disp = max(trans_disps) if trans_disps else 1.0
        
    #     # Make sure max_disp is not too small to avoid division issues
    #     if max_disp < 1e-10:
    #         max_disp = 1.0
        
    #     # Normalize and scale
    #     for key, value in list(displacements.items()):
    #         if value is not None:
    #             displacements[key] = (value / max_disp) * scale
        
    #     # Explicitly print some of the displacements to verify
    #     print("Normalized and scaled displacements for some nodes:")
    #     for node_id in list(self.nodes.keys())[:3]:  # First 3 nodes
    #         print(f"Node {node_id}:")
    #         for dof in range(3):  # First 3 DOFs (translational)
    #             print(f"  DOF {dof}: {displacements.get((node_id, dof), 0.0)}")
        
    #     # Apply these displacements to the model
    #     self.update_state(displacements)
        
    #     # Create a results object for visualization
    #     results = Results(self.assembler, displacements)
